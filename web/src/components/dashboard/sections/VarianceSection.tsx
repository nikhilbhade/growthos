...
export function VarianceSection() {
  ...
  const totalToast = varianceRows.reduce((s, r) => s + r.toast, 0);
  const totalReported = varianceRows.reduce((s, r) => s + r.reported, 0);
  const variance = ((totalReported - totalToast) / totalToast) * 100;
  ...
  return (
    <div className="space-y-6">
      <SectionHeading
        ...
        description="Compare Toast completed-order values with each source's reported or attributed value, then see the reason a difference is expected."
        right={<ReadOnlyBadge>Directional · read only</ReadOnlyBadge>}
      />

      <InfoNotice>
        Toast is the financial source of truth. Provider metrics use their own reporting windows and attribution rules, so they will not
        always reconcile exactly.
      </InfoNotice>

      <div className="grid gap-4 sm:grid-cols-3">
        <StatTile label="Toast completed orders" value={money(totalToast)} hint="financial source of truth" />
      ...
      <Card>
        <CardContent className="p-0">
          <div className="border-b border-border px-5 py-3">
            <p className="text-sm font-medium">Toast vs. source · reconciliation by source</p>
          </div>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {varianceRows.map((row) => {
                const v = ((row.reported - row.toast) / row.toast) * 100;
                return (
                  <TableRow
                    key={row.id}
                  ...
                  >
                    ...
                    <TableCell className="text-right tabular-nums text-muted-foreground">{money(row.toast)}</TableCell>
          ...
          {selected && (
            <>
              <SheetHeader>
                ...
                <SheetDescription>Why Toast and this source differ for the selected period.</SheetDescription>
              </SheetHeader>
              <div className="space-y-4 px-6 pb-6">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-lg border border-border bg-background/50 p-4">
                    <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Toast</p>
                    <p className="mt-1 text-xl font-semibold tabular-nums">{money(selected.toast)}</p>
                  </div>
                  <div className="rounded-lg border border-border bg-background/50 p-4">
                ...
                <div className="space-y-2 text-sm text-muted-foreground">
                  <p>
                    <strong className="text-foreground">Toast:</strong> completed orders and sales after the selected reporting cutoff.
                  </p>
                  <p>
