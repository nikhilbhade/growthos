...
const providerInfo = {
  ...
  doordash: { name: 'DoorDash', label: 'Merchant Portal', logo: 'DD', authorization: 'Open Merchant Portal user management and invite the GradientOS integration identity.', requirement: 'The restaurant’s Business Admin, or the Business Group Admin for multi-brand groups.', read: ['Restaurant locations and store details', 'Location mapping and connection health'], history: 'We match the stores that appear to the locations you expect to see.', scope: 'Choose the brands or business groups GradientOS should include.', validation: 'We check that the right restaurant locations came through.', eta: 'About 5 minutes for you · up to 15 minutes for GradientOS to check the first sync' },
  ubereats: { name: 'Uber Eats', label: 'Merchant Portal', logo: 'UE', authorization: 'Open Uber Eats Manager and invite the GradientOS integration identity as Manager.', requirement: 'The person who manages access for each restaurant location in Uber Eats Manager.', read: ['Restaurant locations and store details', 'Location mapping and connection health'], history: 'We match the stores that appear to the locations you expect to see.', scope: 'Choose the restaurant locations GradientOS should include.', validation: 'We check that the right restaurant locations came through.', eta: 'About 5 minutes for you · up to 15 minutes for GradientOS to check the first sync' }
};
...
function focusCard(connection) {
  ...
  return `<article class="connection-card ${isPaidMedia ? 'connection-card-focus' : ''}"><div class="connection-head">${providerLogo(connection.id)}<div><h3>${connection.name}</h3><small>${info.label}</small></div><span class="integration-state state-${connection.status}">${connection.status.replace('_', ' ')}</span></div><div class="connection-guidance"><div><span>WHO SHOULD DO THIS</span><p>${info.requirement}</p></div><div><span>WHAT HAPPENS NEXT</span><p>They sign in, choose the restaurant account(s), and GradientOS checks the first sync.</p></div></div><p class="connection-eta">${info.eta}</p><button class="connection-action" data-connect="${connection.id}"><span>${actionLabel}</span><i aria-hidden="true">→</i></button></article>`;
}
