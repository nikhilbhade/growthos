(function initialiseWorkflows() {
  ...
  const suggested = [
    { id: 'meta-tiktok-cpa', title: 'Shift Meta prospecting to TikTok', subtitle: 'Ideal to improve new-guest transactions', window: 'Last 7 days', trigger: 'Meta prospecting CPA is at least 20% above target with at least $1,500 in spend.', source: ['meta'], destination: ['tiktok'], amount: 'Reduce only the affected top-of-funnel budget; start TikTok at its minimum viable daily budget.', protect: 'Keep Meta retargeting fully funded: 180-day site visitors, cart abandoners, and add-to-cart audiences.', setup: 'TikTok Spark Ads · 15-day engagement audience · interest and behavior layering · exclude 30-day purchasers.', reset: 'Return budget when Meta CPA is back within target for the following trailing window.', creative: ['TikTok Spark Ads — partner with 2–3 local food creators; 9:16 native video with the hook in the first 2 seconds.', 'Show the dish being made and eaten, with on-screen text carrying the first-order offer.', 'Launch 3 hooks × 2 offers and refresh every 7–10 days to stay ahead of fatigue.'], targeting: ['Start broad with Smart+ / interest + behavior layering (food delivery, dining out) and let TikTok optimize.', 'Add a 15-day video-engagement custom audience as the warm retargeting layer.', 'Exclude 30-day purchasers; geo-fence to the delivery radius around each location.'], status: 'Needs CPA target' },
    { id: 'roas-ranking', title: 'Rotate budget to the strongest blended ROAS', subtitle: 'Ideal to grow contribution-efficient sales', window: 'Every Monday · last 7 days', trigger: 'Rank Meta, TikTok, and Google by blended ROAS; identify the lowest and highest channel.', source: ['meta', 'tiktok', 'google'], destination: ['meta', 'tiktok', 'google'], amount: 'Move 10–15% of total budget from the lowest-ranked channel to the highest; cap each platform at ±20% week over week.', protect: 'Protect high-intent and retargeting activity on the source channel before moving any prospecting dollars.', setup: 'Meta: Advantage+ Shopping or broad · TikTok: Smart+ · Google: Performance Max with Customer Match and in-market signals.', reset: 'Re-rank next Monday; reverse a move when the channel ordering changes.', creative: ['Meta: Advantage+ creative mix — UGC video, a menu-highlight carousel, and one value/offer static.', 'TikTok: Smart+ creator-led vertical video; keep 3+ active variants running.', 'Google: Performance Max — supply a full asset group (video, images, headlines, sitelinks).'], targeting: ['Meta: broad / Advantage+ Shopping optimized for purchases.', 'TikTok: Smart+ audience with interest + behavior signals.', 'Google: Performance Max with Customer Match lists and in-market audience signals.'], status: 'Ready to configure' },
    { id: 'retargeting-saturation', title: 'Relieve Meta retargeting saturation', subtitle: 'Ideal to improve repeat transactions', window: 'Last 7 days', trigger: 'A Meta retargeting ad set reaches frequency above 6, or its audience falls below the chosen reach floor.', source: ['meta'], destination: ['tiktok', 'google'], amount: 'Reduce only the saturated Meta retargeting ad set and preserve Meta prospecting.', protect: 'No changes outside the individual saturated retargeting layer.', setup: 'TikTok: last-14-day ViewContent/AddToCart audience · Google: RLSA on high-intent search plus YouTube remarketing.', reset: 'Restore Meta budget once frequency and reach normalize for a full trailing window.', creative: ['TikTok: fresh 9:16 video for warm viewers — a new dish or limited-time offer to reset fatigue.', 'Google: RLSA responsive search ads plus short YouTube remarketing bumpers.', 'Avoid reusing the saturated Meta creative; give the warm audience a new angle.'], targeting: ['TikTok: last-14-day ViewContent / AddToCart engagers.', 'Google: RLSA on high-intent branded and non-branded search; YouTube remarketing to site visitors.', 'Cap frequency and rotate the audience pool weekly.'], status: 'Needs reach floor' },
    { id: 'demand-capture', title: 'Capture a demand spike in Google Search', subtitle: 'Ideal to grow sales from high-intent demand', window: 'Daily check · 7-day context', trigger: 'Organic sessions rise at least 30% day over day, or branded search volume spikes after a launch, promo, or press mention.', source: ['meta', 'tiktok'], destination: ['google'], amount: 'Temporarily trim paid-social prospecting by the chosen percentage for the duration of the demand spike.', protect: 'Do not trim Meta or TikTok retargeting layers.', setup: 'Fund exact-match branded search first, then non-branded Target ROAS search and an isolated high-intent Performance Max asset group.', reset: 'Restore paid-social prospecting after the demand signal returns to baseline.', creative: ['Exact-match branded responsive search ads with sitelinks, callouts, and location extensions.', 'Non-branded high-intent search with offer-led headlines.', 'An isolated Performance Max asset group with menu imagery and a 15s vertical video.'], targeting: ['Prioritize branded and high-intent non-branded keywords with Target ROAS bidding.', 'Layer in-market “restaurants / food delivery” audience signals.', 'Geo-target the delivery radius and daypart to lunch/dinner peaks.'], status: 'Needs baseline' },
    { id: 'delivery-shift', title: 'Move paid-social budget into delivery marketplaces', subtitle: 'Ideal to grow marketplace payouts', window: 'Last 14 days', trigger: 'Combined Meta and TikTok spend exceeds $4,000 during the trailing window.', source: ['meta', 'tiktok'], destination: ['doordash', 'ubereats'], amount: 'Shift $2,000 total, allocated by historical DoorDash and Uber Eats order share.', protect: 'Reduce prospecting only; retain paid-social retargeting and high-intent layers.', setup: 'DoorDash and Uber Eats: separate new-customer and lapsed-customer campaigns; lunch and dinner dayparts; platform-specific offers.', reset: 'Revert when social spend falls below $4,000 or blended delivery CAC exceeds your configured ceiling.', creative: ['DoorDash / Uber Eats sponsored-listing creative with hero dish photography and a clear offer badge.', 'Separate new-customer ($ off first order) and lapsed-customer win-back creative.', 'Keep offers platform-specific rather than reusing paid-social assets.'], targeting: ['Run separate new-customer and lapsed-customer campaigns on each marketplace.', 'Split lunch and dinner dayparts with platform-specific offers.', 'Weight budget by historical DoorDash vs. Uber Eats order share.'], status: 'Needs CAC ceiling' }
  ];
  ...
  function renderPanel() {
    ...
    const recBlock = (item.creative || item.targeting)
      ? `<div class="workflow-recommendations"><div class="workflow-rec-card workflow-rec-creative"><span>◈ RECOMMENDED CREATIVE</span><ul>${(item.creative || []).map(entry => `<li>${esc(entry)}</li>`).join('')}</ul></div><div class="workflow-rec-card workflow-rec-targeting"><span>◎ RECOMMENDED TARGETING</span><ul>${(item.targeting || []).map(entry => `<li>${esc(entry)}</li>`).join('')}</ul></div></div>`
      : `<div class="workflow-recommendations workflow-recommendations-empty"><span>◈ CREATIVE &amp; TARGETING</span><p>Add creative and targeting guidance for this workflow during review, or start from a suggested playbook to inherit recommended setups.</p></div>`;
    panel.innerHTML = `<div class="workflow-canvas-head"><div><span class="eyebrow">${isCustom ? 'CUSTOM WORKFLOW' : 'SUGGESTED PLAYBOOK'}</span><h3>${esc(item.title)}</h3><p>${esc(item.subtitle || 'Designed around your selected business outcome')}</p></div><span class="workflow-status ${item.status.includes('Needs') ? 'workflow-status-warning' : ''}">${esc(item.status)}</span></div><div class="workflow-canvas-meta"><span><b>OUTCOME</b>${esc(item.subtitle || 'Custom outcome')}</span><span><b>WINDOW</b>${esc(item.window)}</span><span><b>MODE</b>Advisory · review required</span></div>${diagram(item)}<div class="workflow-explainer"><div><span>PROTECTED LAYERS</span><p>${esc(item.protect || 'Source high-intent and retargeting layers remain protected unless an authorized reviewer approves an exception.')}</p></div><div><span>DESTINATION SETUP</span><p>${esc(item.setup || 'Use the delivery configuration agreed during review.')}</p></div></div>${recBlock}<footer class="workflow-canvas-footer"><span><b>Review route:</b> ${esc(item.reviewer || 'Highest-sales channel owner')}</span>${isCustom ? `<button type="button" class="workflow-delete" data-delete-workflow="${item.id}">Remove draft</button>` : `<button type="button" class="workflow-draft" data-suggested-workflow="${item.id}">Add to drafts <b>→</b></button>`}</footer>`;
