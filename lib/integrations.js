...
const demoIntegrations = [
  ...
  { id: 'toast', provider: 'toast', name: 'Toast', category: 'POS and orders', required: false, status: 'needs_setup', freshness: 'Not connected', coverage: 'Online-order sales and location data', fields: ['order ID', 'order time', 'location', 'order channel', 'net sales', 'discounts', 'customer reference (hashed, if authorized)', 'verified attribution identifiers'], note: 'Connect Toast to normalize completed orders and reconcile platform attribution against financial outcomes. A direct order match is enabled only after a checkout identifier bridge is verified.' },
  { id: 'pos', provider: 'pos', name: 'Other POS', category: 'POS and orders', required: false, status: 'needs_setup', freshness: 'Not connected', coverage: 'Custom POS or order-system import', fields: ['order ID', 'order time', 'location', 'net sales', 'discounts', 'order channel', 'verified attribution identifiers'], note: 'Use this for Square, NCR, QSRSoft, or another order system. GrowthOS reconciles platform attribution to POS outcomes first; direct order matching is optional and requires validated identifiers.' },
  { id: 'doordash', provider: 'doordash', name: 'DoorDash', category: 'Marketplace', required: false, status: 'needs_setup', freshness: 'Pending access invite', coverage: 'Merchant Portal · brand or business-group access', fields: ['store access', 'store metadata', 'location mapping', 'ingestion health'], note: 'Invite the GrowthOS integration identity as Business Admin or Business Group Admin. One grant should cover every selected brand.' },
  { id: 'ubereats', provider: 'ubereats', name: 'Uber Eats', category: 'Marketplace', required: false, status: 'needs_setup', freshness: 'Pending access invite', coverage: 'Merchant Portal · store-by-store access', fields: ['store access', 'store metadata', 'location mapping', 'ingestion health'], note: 'Invite the GrowthOS integration identity as Manager for every restaurant location. Each store must be added individually.' }
];
...
function demoWorkspaceIntegrations() {
  return demoIntegrations.map(item => ({
    ...
    coverage: ({ meta: 'Facebook + Instagram · 2 campaigns', tiktok: 'TikTok Ads · 2 campaigns', google: 'Search + YouTube · 3 campaigns', toast: 'Toast · 3 preview locations', pos: 'Order import · 3 preview locations', doordash: 'Merchant Portal · 3 preview locations', ubereats: 'Merchant Portal · 3 preview locations' })[item.id],
...
async function listIntegrations({ demo = false } = {}) {
  ...
  const { data, error } = await supabase.from('integration_connections').select('*').in('provider', ['meta', 'tiktok', 'google', 'toast', 'pos', 'doordash', 'ubereats']);
