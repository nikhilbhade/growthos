...
function buildTools({ agent, memory, demo = false, defaultRange = 'last_14_days', trace }) {
  ...
  const lookupMetric = tool(async ({ term }) => {
    ...
    if (!resolved) return `"${term}" is not a recognized metric in the GradientOS registry.`;
    if (trace.metricVersion == null) trace.metricVersion = resolved.version;
    return JSON.stringify(resolved);
  }, {
    name: 'lookup_metric_definition',
    description: 'Resolve a metric term (e.g. "results", "CPA", "ROAS") to its GradientOS definition, provider field, and attribution window. Use before answering when a metric is ambiguous.',
  ...
  const searchKnowledgeTool = tool(async ({ query }) => {
    ...
    description: 'Search GradientOS documentation (metric definitions, data dictionary, naming taxonomy, integration SOP) to ground an answer. Use for "how does X work", attribution, account structure, or connection questions.',
  ...
  const refuse = tool(async ({ reason }) => {
    trace.refusalReason = reason || 'Requested a change GradientOS agents cannot make.';
    trace.refused = true;
