...
function planQuery({ message = '', dimension = 'campaign', range = 'last_14_days' }) {
  if (blockedActionPattern.test(message)) {
    return { safe: false, intent: 'mutation_request', tool: null, dimension, range, reason: 'GradientOS retrieval agents cannot change provider settings.' };
  }
...
function summarize(retrieval, plan, message = '') {
  if (retrieval.pending) return {
    answer: `I’d like to pull that for you, but ${retrieval.agent.provider} is not connected to GradientOS yet. I don’t want to guess at account data. Once the account is authorized and the first sync is validated, I can retrieve and explain the campaigns, structure, and delivery metrics in this chat.\n\nAnswer basis\n• Requested scope: ${objectLabel(plan.dimension, retrieval.agent.provider)} data for ${readableRange(plan.range)}\n• Retrieval status: connection required before any account data can be read`,
    findings: [],
    suggestedPrompts: ['Which person should connect this account?', 'What data will GradientOS read?']
  };
...
async function runAgentChat({ agent, message, dimension, range, demo, brandId = null, threadId = null }) {
  ...
  const { config, modelEnabled } = require('./config');
  if (!modelEnabled()) return runDeterministic({ agent, message, dimension, range, demo, runId });

  // Try the LangGraph agent; fall back to deterministic on any failure so the
  // chat (and preview mode) never breaks.
  try {
    if (config.model.provider === 'vertex') {
      const { runVertexAgent } = require('./vertex-agent');
      return await runVertexAgent({ agent, message, plan: guardPlan, demo, brandId, threadId });
    }
